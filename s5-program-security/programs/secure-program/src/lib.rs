...
declare_id!("Cahw9yJvjRbX7XhbphQG97XCNVA4nJZYuwew33Ca4LpZ");
...
pub mod secure_program {
    ...
    pub fn initialize(ctx: Context<CreateUser>, id: u32, name: String) -> Result<()> {
        ...
        require!(user.owner == *ctx.accounts.signer.key, MyError::Unauthorized);
    ...
    pub fn transfer_points(ctx: Context<TransferPoints>, amount: u16) -> Result<()> {
        require!(ctx.accounts.sender.points >= amount, MyError::NotEnoughPoints);

        ctx.accounts.sender.points.checked_sub(amount).unwrap();
        ctx.accounts.receiver.points.checked_add(amount).unwrap();
